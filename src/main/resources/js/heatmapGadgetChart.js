...
function redrawChartToDrilldown(data) {
    ...
    gadgets.window.adjustHeight();
            ...
            function recalculateSize(results, summ){
                var count = 0;
                var cellsNumber = AJS.$(this)[0].gadgets.Prefs().getString('cellsNumber');
                results.forEach(function(projectInfo){
                    var riskScore = projectInfo.riskScore;
                    var calculatedValue = parseInt((riskScore * cellsNumber) / summ);
                    if (calculatedValue < 1){
                        calculatedValue++;
                        count++;
                    }
                    projectInfo.value = calculatedValue;
                });
                if(count > 0 ){
            ...
            function correctionsInCalculations(results, count){
                 var summ = 0;
                 var overallWeight = 0;
                 results.forEach(function(projectInfo){
                    summ+=projectInfo.value;
                 });
                 results.forEach(function(projectInfo){
                    var squareSize = projectInfo.value;
                    if (squareSize !=1){
                        overallWeight += squareSize/(summ - squareSize);
                    }
                 });
                 ...
                 results.forEach(function(projectInfo){
                    ...
                    if(squareSize !=1 && overallWeight>0 && count >0){
                        var valueToDecrement = Math.round((count * (squareSize/(summ - squareSize)))/overallWeight);
                        overallWeight-=valueToDecrement;
                        count--;
                        projectInfo.value = squareSize - valueToDecrement;
                    }
                 });
            }

            function setMinimumValueToRender(results){
                if(Array.isArray(results) && results.length>1){
                    var summ = 0;
                    var values = [];
                    results.forEach(function(projectInfo){
                        var valueToAdd = projectInfo.riskScore;
                        if (valueToAdd==0){
                            valueToAdd++;
                        }
                    ...
                    var isTrue =  (minValue/summ) <= (1/ AJS.$(this)[0].gadgets.Prefs().getString('cellsNumber'));
                    if( (minValue/summ) <= (1/ AJS.$(this)[0].gadgets.Prefs().getString('cellsNumber')) ){
                       recalculateSize(results, summ);
                    }
                }
            }
